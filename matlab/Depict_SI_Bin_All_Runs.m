...
RasterPlot  = ManySlotBuffer(nStim*nHippo,nMaxSample,nMaxTime);    
    ...
    FIndex=W12_MAXIMUM_Binary((iRun-1)*nHippo+1:(iRun*nHippo));
...
% *************************************************************************
%%
WeightLayer1To2_NN     = zeros(nRun,nTrial,nIn,nHippo);
WeightLayer2To3_NN     = zeros(nRun,nTrial,nHippo,nOut);
WeightLayer1To2        = zeros(nRun,nTrial,nIn,nHippo);
WeightLayer2To3        = zeros(nRun,nTrial,nHippo,nOut);
load WeightLayer1To2_data.txt;
load WeightLayer2To3_data.txt;
for i = 1:nTrial*nRun,
    t=(i-1)*50+1;
    WeightLayer1To2_PerTrial=WeightLayer1To2_data(t:t+49);
    iRun=WeightLayer1To2_PerTrial(1);
    iTrial=WeightLayer1To2_PerTrial(1+1);
    for m = 1 : nIn,
        for n = 1 : nHippo,
            WeightLayer1To2_NN(iRun,iTrial,m,n) = WeightLayer1To2_PerTrial(1+1+(m-1)*nHippo+n);
        end
    end
...
for i = 1 : nRun,
    for j = 1 : nTrial,
            max_row=max(WeightLayer1To2_NN(i,j,:,:));
            max_col=max(max_row);
            WeightLayer1To2(i,j,:,:)=WeightLayer1To2_NN(i,j,:,:)/max_col;
    end
end
...
for i = 1:nTrial*nRun,
    tt=(i-1)*18+1;
    WeightLayer2To3_PerTrial=WeightLayer2To3_data(tt:tt+17);
    iRun=WeightLayer2To3_PerTrial(1);
    iTrial=WeightLayer2To3_PerTrial(1+1);
    for mm = 1 : nHippo,
        for nn = 1 : nOut,
            WeightLayer2To3_NN(iRun,iTrial,mm,nn) = WeightLayer2To3_PerTrial(1+1+(mm-1)*nOut+nn);
        end 
    end
end   
for i = 1 : nRun,
    for j = 1 : nTrial,
            max_row=max(WeightLayer2To3_NN(i,j,:,:));
            max_col=max(max_row);
            WeightLayer2To3(i,j,:,:)=WeightLayer2To3_NN(i,j,:,:)/max_col;
    end
end
